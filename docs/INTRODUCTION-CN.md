...
> **重要演示约束**
> SalaryCipher 会在链上严格执行发薪周期。为了确保演示流程稳定通过，请在添加员工前先把系统时间调整到上个月，员工添加完成后再把系统时间调回当前时间。
>
> 创建公司时，选择的发薪日期一定不要早于当前月份的当天日期。如果配置的发薪日已经早于当前日期，当前工资周期仍会被视为尚未结束，`Run Payroll Now` 会被阻止，直到该工资周期结束后才能执行。
...
| 角色     | 权限                                                                                               |
| -------- | -------------------------------------------------------------------------------------------------- |
| Owner    | 创建公司、管理员工、配置发薪日、执行发薪、管理金库、查看公司级历史、参与调薪谈判、执行公司合规审计 |
| HR       | 管理员工、查看人员和薪资相关数据、执行部分薪资管理操作、执行公司合规审计、生成自己的薪资证明       |
| Employee | 查看自己的薪资、余额和历史记录，发起自己的调薪谈判，生成自己的薪资证明，unwrap 已发薪资            |
...
### 4.11 Compliance 与 Salary Proofs

当前版本把公司合规和员工个人薪资证明拆成两个页面：

- Compliance：只面向 Owner / HR，当前只包含薪资公平审计。合约在密文下计算薪资分布和差距结论，不暴露任何个人薪资明细。
- Salary Proofs：只面向 HR / Employee，Owner 不显示该页面。用户只能生成、查看、撤销、授权和 mint 自己的薪资证明，不能查看其他员工的证明。

Salary Proofs 当前固定支持三种证明类型：月薪大于等于 X、月薪位于 X 到 Y 之间、在职时长大于等于 N 个月。员工可以把证明 mint 成 RWA NFT。前端会生成 SVG 凭证图并上传 IPFS，再生成 NFT metadata 上传 IPFS，最后把 metadata 的 `tokenURI` 写入 NFT。SVG 和 metadata 不包含真实薪资、员工姓名和解密结果，第三方需要通过链上证明状态和员工授权验证结论。
...
| `SalaryProof`          | 生成收入证明，保存加密验证结果，控制授权、撤销和 NFT mint                         |
| `ProofNFT`             | 把收入证明铸造成 RWA NFT，保存 proofId 与 tokenURI                                |
...
| `useCompanyEmployees`    | 员工列表、添加员工、编辑员工基础信息、删除员工、初始月薪加密与薪资解密 |
...
- Owner / HR 能看公司管理所需数据。
- 员工不能看其他员工薪资。
- Owner 不能查看员工个人薪资证明页面，HR 只能查看自己的薪资证明。
...
- 薪资公平审计。
- RWA 薪资证明。
- Proof NFT、IPFS SVG 凭证图和 IPFS metadata。
- 自动化发薪执行。
- 大规模员工分页发薪。
