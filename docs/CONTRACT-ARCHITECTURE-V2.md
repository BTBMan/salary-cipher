...
| 薪资证明 NFT SVG 图片 | 低     | IPFS             | NFT 展示图由前端生成并上传 IPFS           |
...
- 铸造前先由前端生成 SVG 凭证图并上传 IPFS，得到 `imageURI`
- 再将证明元数据（证明类型、有效期、公司名、结算资产等）上传 IPFS
- 获得 metadata JSON 的 CID 作为 `tokenURI` 传入铸造函数
- metadata 内容示例：

```json
{
  "name": "SalaryCipher Income Proof #001",
  "description": "A privacy-preserving RWA salary proof generated by SalaryCipher.",
  "image": "ipfs://<svg-image-cid>",
  "attributes": [
    { "trait_type": "Proof Type", "value": "Monthly Salary >= Threshold" },
    { "trait_type": "Company", "value": "Acme Corp" },
    { "trait_type": "Settlement Token", "value": "USDC" },
    { "trait_type": "Expiry", "value": "2026-07-06" }
  ],
  "external_url": "https://salarycipher.app/salary-proofs/1"
}
```

注意：metadata 和 SVG 中**不包含真实薪资、员工姓名、解密结果**。第三方需要通过 `SalaryProof` 的链上状态和员工授权验证证明是否有效。

### Hackathon 阶段简化策略

- **Logo 和头像**：直接跳过 IPFS 上传，前端用钱包地址自动生成 Jazzicon 图案代替，零成本零依赖
- **员工显示名称**：保留，存链上明文
- **薪资证明 NFT SVG + metadata**：必须实现 IPFS 上传，因为 NFT 铸造流程需要 tokenURI，RWA 凭证也需要可展示图片
...
存储：
  ...
  proofId → Proof {
    companyId,
    employeeAddr,
    ProofType: MONTHLY_GTE / MONTHLY_BETWEEN / EMPLOYMENT_DURATION_GTE,
    ebool    result,
    uint256  createdAt,
    uint256  expiry,
    bool     revoked,
...
核心函数：
  generateProof(companyId, ProofType, einput thresholdMin, einput thresholdMax, inputProof, validityDays)
    → HR/Employee 本人
    → Owner 不能生成薪资证明，HR 只能生成自己的薪资证明
  ...
  mintNFT(proofId, tokenURI)
    → HR/Employee 本人，要求 result 已生成且 minted == false
    → tokenURI 指向 IPFS metadata，metadata.image 指向 IPFS SVG
    → 调用 ProofNFT.mint()
...
| ProofType | 说明 |
| --------- | ---- |
| `MONTHLY_GTE` | 月薪大于等于加密阈值 |
| `MONTHLY_BETWEEN` | 月薪位于加密区间 |
| `EMPLOYMENT_DURATION_GTE` | 在职时长大于等于指定月数 |
...
核心函数：
  mint(to, proofId, tokenURI) → tokenId
    → 仅 SalaryProof 合约可调用
    → 一个 proofId 只能 mint 一个 NFT
    → tokenURI 必须是已上传 IPFS 的 metadata URI
...
```
HR/Employee → SalaryProof.generateProof()
              ↓ 内部调用
           SalaryCipherCore.verifySalaryCondition()           // 薪资比较在 Core 内完成
HR/Employee → SalaryProof.authorizeVerifier(verifierAddr)     // 授权第三方
前端     → 生成 SVG 凭证图并上传 IPFS，得到 imageURI
前端     → 生成 metadata JSON 并上传 IPFS，得到 tokenURI
HR/Employee → SalaryProof.mintNFT(proofId, tokenURI)         // 可选：铸造 RWA NFT
              ↓ 内部调用
           ProofNFT.mint()
