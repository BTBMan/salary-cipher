...
核心函数：
  generateProof(companyId, ProofType, euint256 thresholdMin, euint256 thresholdMax, uint256 validity)
    → Employee 本人
    → Owner 不能生成薪资证明，HR 只能生成自己的薪资证明
    → 从 ConfidentialPayroll 读取 monthlySalary
    → result = TFHE.ge(monthlySalary, threshold)
    → 写入 Proof
  mintProofNFT(proofId, tokenURI)
    → HR/Employee 本人，将 Proof 铸造为 ERC721
...
HR/Employee → SalaryProof.mintProofNFT(tokenURI)（可选）
